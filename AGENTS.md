...
9a. **Concrete tasks → `@code-worker` subagent (preferred)**: the model
    ...
    `~/.config/opencode/agents/code-worker.md` and operates as a strict executor.
    ...
    where "figuring out what to do" is most of the job. `@code-worker` may
...
9b. **Git operations → `@git-maintainer` subagent**: any git/GitHub
    ...
    Defined at `~/.config/opencode/agents/git-maintainer.md` (model:
